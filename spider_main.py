...
def start_topic_page_spider(account, start_index):
    mongo = db_tool.create_mongo_session()
    topic_session = mongo.get_database('xiaomuchong_db').get_collection('topic')
    ...
    for page_index in tqdm(range(start_index, start_index + 20, 1)):
        ...
        driver.get(url=topic_page_url)
        topic_items = driver.find_elements_by_class_name(name='forum_list')
